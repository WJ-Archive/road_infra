...
class LoadStreams:
    ...
    def __next__(self):
        ...
        im0 = self.imgs.copy()
        print(f"dataloader im0 size :{len(im0[0])}x{len(im0[0][0])}x{len(im0[0][0][0])}")
...
class Custom_LoadStreams:
    def __init__(self, sources='file.streams', img_size=640, stride=32, auto=True, transforms=None, vid_stride=1, cam_width=640, cam_height=640):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            self.imgs[i], self.fps[i] = self.cam.capture() # guarantee first frame
