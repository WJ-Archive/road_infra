...
from yolov5.utils.dataloaders import VID_FORMATS, LoadImages, LoadStreams, Custom_LoadStreams
...
def create_shared_block():
    ...
    a = np.ones(shape=(CAM_WIDTH,CAM_HEIGHT,3), dtype = np.int64)
...
def pack_data(cls_name, cls, conf, xyxy, fc):
    ...
    #xyxy = xyxy.view(-1).tolist()
    #xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)))
    #xywh = xywh.view(-1).tolist()
    print(cls_name, cls, conf, xyxy, fc)
    bbox_x = xyxy[0]
    bbox_y = xyxy[1]
...
# add. Message Send(UDP)
def socket_send(sock, msg):
    sock.sendto(msg.encode(), (MAIN_SERVER_IP, MAIN_SERVER_UDP_PORT))
    print("udp send")

"""
#Frame Send (UDP) X 
def send_frame_udp(frame):
    d = frame.flatten()
    s = d.tostring()

    for i in range(20):
        sock_udp.sendto(s[i *46080:(i+1)*46080], ("192.168.0.244", 3838))
"""
...
def run(
    ...
    else:  # multiple models after --yolo_weigh
    ...
    #imgsz = check_img_size(imgsz, s=stride)  # check image size (best.pt trained => 640)
    imgsz = IMG_SIZE
    ...
    np_sh_frame = np.ndarray((CAM_HEIGHT,CAM_WIDTH,3), dtype=np.int64, buffer = sh_frame.buf)
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        #print(f"im0s size : {len(im0s)}x{len(im0s[0])}x{len(im0s[0][0])}x{len(im0s[0][0][0])}")
        with dt[0]:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            np_sh_frame[:] = np_im0[:]
            sem.release()
            #print("@@@@@@@@@@@@@@@@@@2",len(np_sh_frame),len(np_sh_frame[0]),len(np_sh_frame[0][0]))
            ...
            if show_vid:
                if platform.system() == 'Linux' and p not in windows:
                    ...
                    print("size?",im0.shape[1], im0.shape[0])
...
if __name__ == "__main__":
    ...
    # add. Create Shared_Memory
    a = np.ones(shape=(CAM_WIDTH,CAM_HEIGHT,3), dtype = np.int64)
    shm = shared_memory.SharedMemory(create=True, size=a.nbytes)
    np_shm = np.ndarray(a.shape, dtype=np.int64, buffer=shm.buf)
    np_shm[:] = a[:] #Copy the original Data into Shared memory

    opt = parse_opt()
    
    # run yolo & UDP msg send (process1)
    p1 = Process(target=main, args=(opt,))
    p1.start()
    print("p1 start")
